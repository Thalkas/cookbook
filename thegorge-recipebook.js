...
function GetDishes() {
    $.ajax({
        type: 'GET',
        url: "https://raw.githubusercontent.com/Thalkas/cookbook/master/public/recipedata.json",
...
function SelectDish(dish_elem) {
    ...
    if (dish_data) {
        ...
        var first = true;
        var empty = true;
        for (var coin_index = 0; coin_index < 8; coin_index++) {
        ...
        $(".recipedetails .dish-tribute .empty").text((index == 72) ? "None" : "Unknown");
        if (empty) {
        ...
        $(".recipedetails .dish-plate .empty").text((index == 72) ? "None" : "Unknown");
        if (empty) {
        ...
        if (cravings_text.length == 0) {
            cravings_text = (index == 72) ? "None" : "Unknown";
        } else {
